...
def main():
    ...
    if args.mode == 'train':
        ...
        train_model(args.dataset, args.save_path)
...
def train_model(dataset_path, save_path):
    ...
    # Training
    train_losses, val_losses, train_accs, val_accs = train_and_evaluate(model, train_loader, val_loader, criterion, optimizer, num_epochs, device)

    # Save the model
    if not os.path.basename(save_path):
        save_path = os.path.join(save_path, "part8.pth")
...
def run_inference(dataset_path, model_path):
    ...
    # Check if model_path has a filename at the end
    if not os.path.basename(model_path):
        model_path = os.path.join(model_path, "part8.pth")
    ...
    print("answer_vocab_path", answer_vocab_path)
    # Load answer vocabulary
    if os.path.isfile(model_path):
        answer_vocab_path = os.path.join(os.path.dirname(model_path), 'answer_to_id.json')
    else:
    ...
    # Check if the model path includes a filename, if not, append a default filename
    if not os.path.basename(model_path):
        model_path = os.path.join(model_path, "part8.pth")
    model = VQAModel(tokenizer.vocab_size, num_classes, max_len=max_len).to(device)

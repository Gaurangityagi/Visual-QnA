...
# Modified TextEncoder with BERT embeddings
class TextEncoderWithBERT(nn.Module):
    def __init__(self, vocab_size, embed_dim=768, max_len=50, num_layers=6, nhead=8):
        super(TextEncoderWithBERT, self).__init__()
        # Load BERT model to get the embedding weights
        bert = BertModel.from_pretrained('bert-base-uncased')
        bert_embeddings = bert.embeddings.word_embeddings.weight  # Shape: [vocab_size, embed_dim]
        
        # Initialize nn.Embedding with BERT embeddings
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        with torch.no_grad():
            self.embedding.weight.copy_(bert_embeddings)
        self.embedding.weight.requires_grad = False  # Freeze the embeddings
        logger.info("Initialized TextEncoder with BERT embeddings (frozen)")
...
# Modified VQAModel to use TextEncoderWithBERT
class VQAModelWithBERT(VQAModel):
    def __init__(self, vocab_size, num_classes, embed_dim=768, max_len=50):
        super(VQAModelWithBERT, self).__init__(vocab_size, num_classes, embed_dim, max_len)
        self.text_encoder = TextEncoderWithBERT(vocab_size, embed_dim, max_len)
        logger.info("Initialized VQAModel with BERT embeddings in TextEncoder")
...
def train_model(dataset_path, save_path, model_path):
    ...
    model = VQAModelWithBERT(tokenizer.vocab_size, num_classes, max_len=max_len).to(device)
    ...
    num_bert_epochs = 2  # Reduced epochs to prevent overfitting
    logger.info(f"Starting training with BERT embeddings for {num_bert_epochs} epochs")
    bert_train_losses, bert_val_losses, bert_train_accs, bert_val_accs = train_and_evaluate(
        model, train_loader, val_loader, criterion, optimizer, num_bert_epochs, device
    )
    # Save the model
    model_save_path = save_path
    # torch.save(model, model_save_path)  # Save the entire model
    torch.save(model.state_dict(), model_save_path)
    print(f'bert trained model saved at: {model_save_path}')
...
def run_inference(dataset_path, model_path):
    ...
    test_img_dir = os.path.join(base_path, 'images/testB')
    test_q_file = os.path.join(base_path, 'questions/CLEVR_testB_questions.json')
    print("2")
    # Load answer vocabulary
    if not os.path.basename(model_path):
        model_path = os.path.join(model_path, "part10b.pth")
    ...
    criterion = nn.CrossEntropyLoss()
    # Run evaluation
    accuracy, precision, recall, f1, all_preds, all_targets = evaluate_test(model, test_loader, criterion, device)
